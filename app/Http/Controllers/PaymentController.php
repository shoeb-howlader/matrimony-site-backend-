...
// ইউজারকে রিডাইরেক্ট করার পর দেখানোর জন্য (Success Page)
    public function success(Request $request)
    {
        ...
        // পেমেন্ট মেথড ক্লিন করার লজিক
        $cardType = $request->input('card_type', 'sslcommerz');
        $paymentMethod = str_contains($cardType, '-') ? explode('-', $cardType)[1] : $cardType;

        // 🔴 বাগ ফিক্স: ট্রানজেকশনটি আগেই কলব্যাক থেকে সফল হয়ে থাকলে নতুন করে কানেকশন বাড়াবো না
        $alreadyProcessed = $transaction->status === 'success';
        ...
        // 🔴 শুধুমাত্র নতুন সফল পেমেন্টের ক্ষেত্রেই নোটিফিকেশন যাবে
        if (!$alreadyProcessed) {

            // ── ৩. অ্যাডমিনদের নোটিফিকেশন পাঠানো ──
            try {
                $admins = \App\Models\User::where('role', 'admin')->get();
                $userName = $transaction->user->name ?? 'একজন ইউজার';
                $adminNotificationData = [
                    'title' => 'নতুন পেমেন্ট গ্রহণ করা হয়েছে!',
                    'message' => "{$userName} সফলভাবে ৳{$transaction->amount} টাকার একটি প্যাকেজ কিনেছেন। ট্রানজেকশন আইডি: {$transactionId}",
                    'link' => '/admin/payments'
                ];
            ...
            // ── 🔴 ৪. ইউজারকে ইমেইল + রিয়েল-টাইম নোটিফিকেশন পাঠানো ──
            try {
                if ($transaction->user) {

                    // প্যাকেজের নামটি বের করা
                    $package = \App\Models\ConnectionPackage::find($transaction->connection_package_id);
                    $packageName = $package ? $package->name : 'কানেকশন প্যাকেজ';

                    // 🔴 আমাদের নতুন তৈরি করা PackagePurchasedNotification কল করা হলো 🔴
                    $notification = new \App\Notifications\PackagePurchasedNotification($transaction, $packageName);

                    // ডাটাবেস এবং ইমেইলে নোটিফিকেশন পাঠানো
                    $transaction->user->notify($notification);

                    // Reverb এর মাধ্যমে রিয়েল-টাইমে ইউজারকে পুশ করা
                    $notificationData = $notification->toArray($transaction->user);
                    $notificationObj = [
                        'id' => \Illuminate\Support\Str::uuid()->toString(),
                        'type' => 'App\\Notifications\\PackagePurchasedNotification', // 🔴 টাইপ আপডেট করা হয়েছে
                        'notifiable_type' => 'App\\Models\\User',
